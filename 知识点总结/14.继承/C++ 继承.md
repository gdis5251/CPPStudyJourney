...
![1.继承的概念](F:\CPPStudyJourney\知识点总结\14.继承\1.继承的概念.png)
...
![2.继承演示](F:\CPPStudyJourney\知识点总结\14.继承\2.继承演示.png)
...
![3.派生类给基类赋值](F:\CPPStudyJourney\知识点总结\14.继承\3.派生类给基类赋值.png)
...
![4.同名成员的隐藏](F:\CPPStudyJourney\知识点总结\14.继承\4.同名成员的隐藏.png)
...
1. **派生类会自动调用基类的默认构造函数来初始化基类的那一部分成员**，**如果基类没有默认构造函数，必须由派生类在初始化列表里显式调用。（先初始化基类，再初始化派生类。)**
2. **派生类的拷贝构造函数必须调基类的拷贝构造函数完成基类的拷贝初始化。如果不显示调用基类的拷贝构造函数，派生类会默认调用基类的构造函数。**
3. **派生类的赋值运算符重载必须调用基类的赋值运算符重载来进行对基类的那一部分成员赋值，如果不调用，编译器也不会给你调用。**
4. **派生类的析构函数会在被调用完成后自动调用基类的析构函数，因为要先清理派生类成员再清理基类成员（栈帧的思想，先定义后清理）。**
...
![5.派生类的成员函数规则](F:\CPPStudyJourney\知识点总结\14.继承\5.派生类的成员函数规则.png)
...
![6.继承的静态成员](F:\CPPStudyJourney\知识点总结\14.继承\6.继承的静态成员.png)
...
1. **单继承：一个子类只有一个直接父类时，称这个继承关系为单继承。**
   ...
   ![7.单继承](F:\CPPStudyJourney\知识点总结\14.继承\7.单继承.png)

2. **多继承：一个子类有两个或以上直接父类时称这个继承关系为多继承**

   ![8.多继承](F:\CPPStudyJourney\知识点总结\14.继承\8.多继承.png)

3. **菱形继承：菱形继承是多继承的一种特殊情况**

   ![9.菱形继承](F:\CPPStudyJourney\知识点总结\14.继承\9.菱形继承.png)
   ...
   ![10.数据重定义](F:\CPPStudyJourney\知识点总结\14.继承\10.数据重定义.png)
   ...
   ![11.菱形继承的存储方式1](F:\CPPStudyJourney\知识点总结\14.继承\11.菱形继承的存储方式1.png)
   ...
   **那么虚拟继承的原理是什么呢？**
   ...
   ![12.菱形继承的存储方式2](F:\CPPStudyJourney\知识点总结\14.继承\12.菱形继承的存储方式2.png)

   这次可以看到，```_g```只有一份了，就是绿色的那个，在最下面。但是我们注意到原来存A B类```_g```的放怎么是看不懂的东西。不，那里存的是地址。那我们到响应的地址看看。

   ![13.菱形虚拟继承](F:\CPPStudyJourney\知识点总结\14.继承\13.菱形虚拟继承.png)
